...
bool RenderWrapper::Initialize() {
    ...
    SDL_Surface *iconSurface = IMG_Load((ConfigSingleton::getInstance().getBaseAssetPath() +
                                         ConfigSingleton::getInstance().getAppLogoPath()).c_str());
    if (iconSurface != nullptr) {
        SDL_SetWindowIcon(window.get(), iconSurface);
        SDL_FreeSurface(iconSurface);
    } else {
        std::cerr << "Icon loading failed: " << IMG_GetError() << std::endl;
    }
